...
class BannersAction extends Action {
    ...
    public function addchannel() {
        //添加kdsbanners与user关联
        if(isset($_POST['kdsbanners']) && !empty($_POST['kdsbanners'])
            && isset($_POST['kdsuseremail']) && !empty($_POST['kdsuseremail'])){
            $kdsbanners = $_POST['kdsbanners'];
            $kdsuseremail = $_POST['kdsuseremail'];
            $ret = file_get_contents(APP_PATH.'Conf/kdsuser.json');
            $kdsuser = json_decode($ret,true);
            foreach ($kdsuser as $k => $v) {
                if($k == $kdsbanners){
                    $kdsuser[$k] = $kdsuseremail;
                }else{
                    $kdsuser[$kdsbanners] = $kdsuseremail;
                }
            }
            $kdsuserjson = json_encode($kdsuser);
            file_put_contents(APP_PATH.'Conf/kdsuser.json', $kdsuserjson);
        }
    ...
    public function getCidsByUid() {
        if (isset($_POST['uid'])) {
            $ret = $this -> _getCids($_POST['uid'], 'banners');
    ...
    private function _getCids($uid, $t) {
        $m = M('admin_channel');
        $ret = $m -> where(array('uid' => $uid, 't' => $t)) -> distinct(true)->field('cid, updatetime')->select();
        return $ret;
    }
    ...
    private function _getTotalStatics($data) {
        ...
        $ret = array(
            ...
            'ctr' => floor(($sum['click'] / $sum['pop']) * 10000) / 10000,
            'profit' => $sum['profit']
        );
        return $ret;
    }
    ...
    private function _addStaticToDB($uid, $cid, $click, $impression, $profit, $date) {
        $t = 'banners';
        $infoitem = array(
            'token' => '',
            'uid' => $uid,
    ...
    public function addstatic() {
        if ($this -> _checklogin()) {
            if (isset($_POST['uid'])) {
                ...
                $returnUrl = U('Banners/info', array('cid' => $cid, 'uid' => $uid));
                $ret = $this -> _addStaticToDB($uid, $cid, $click, $impression, $profit, $date);
                $convertRet = "";
                if ($ret == 3 || $ret == 1) {
