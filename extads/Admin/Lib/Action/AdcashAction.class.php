<?php
// 本类由系统自动生成，仅供测试用途
class AdcashAction extends Action {
    ...
    private function _getUids($t) {
        $m = M('admin_channel');
        $where = array('t' => $t);
        $ret = $m -> where($where) -> order('updatetime') -> distinct(true)->field('uid')->select();
        return $ret;
    }
    ...
    //uptimerobot account huli@hellowd.net
    //uptimerobot password: adonadsautoasync
