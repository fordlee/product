...
class OtherAction extends Action {
    ...
    public function info() {
        if ($this -> _checklogin()) {
            ...
            if ($userid !== 0) {
                $usertypes = $this -> _getUserTypes($userid);
                $this -> assign('types', $usertypes);
            }
            $this -> display();
        } else {
            $this -> redirect('Auth/login');
    ...
    public function getCidsByUid() {
        if (isset($_POST['uid'])) {
            $ret = $this -> _getCids($_POST['uid'], 'other');
    ...
    private function _getUsernameDiscount($ucid) {
        ...
        if ($ucitem !== null) {
            ...
            if ($uitem !== false) {
                ...
                $ret['userid'] = $ucitem['uid'];
            }
        }
        return $ret;
    }

    private function _getUserTypes($uid) {
        $m = M('userchannel');
        $where = array(
            'uid' => $uid
        );
        ...
        if ($this -> _checklogin()) {
            if (isset($_POST['uid']) && isset($_POST['cid'])){
                ...
                if (!empty($uid) && !empty($cid)) {
                    echo $this -> _setStatisticsToLocalDB($uid, $cid, $discount, $mark);
                } else {
                    echo 3;
                }
            } else {
    ...
    private function _setStatisticsToLocalDB($uid, $cid, $discount, $mark) {
        $infom = M('admin_info');
        $limitCount = 20;
        $where = array(
            'uid' => $uid,
    ...
    private function _addStaticToDB($uid, $cid, $impression, $click, $profit, $date, $mark) {
        $t = 'other';
        $infoitem = array(
            'token' => '',
            'uid' => $uid,
    ...
    public function addstatic() {
        if ($this -> _checklogin()) {
            if (isset($_POST['uid'])) {
                ...
                $returnUrl = U('Other/info', array('cid' => $cid, 'uid' => $uid));
                $ret = $this -> _addStaticToDB($uid, $cid, $impression, $click, $profit, $date, $mark);
                $convertRet = "";
                if ($ret == 3 || $ret == 1) {
                    ...
                    if (!is_numeric($discount) || $discount <= 0 || $discount > 1) {
                        ...
                        $conRet = $this -> _setStatisticsToLocalDB($uid, $cid, $discount, $mark);
                        if ($conRet == 1) {
                            $convertRet = '转换成功';
                        } else {
            ...
            if (isset($_FILES['stafile'])
                ...
                if ($failCount == 0) {
                    ...
                    if (!is_numeric($discount) || $discount <= 0 || $discount > 1) {
                        ...
                        $conRet = $this -> _setStatisticsToLocalDB($uid, $cid, $discount, 'AUTO');
                        if ($conRet == 1) {
