...
class AuthAction extends Action {
    public function login() {
        if (isset($_POST['username'])
            ...
            if (($usr == 'adminhw' && $pwd == 'hwadonads2015')
                || ($usr == 'xialinwei' && $pwd == 'hwxlw2015')
                || ($usr == 'liuhanshi' && $pwd == 'hwlhs2015')
                || ($usr == 'durongjian' && $pwd == 'hwdrj2015')
                || ($usr == 'hesiyu' && $pwd == 'hwhsy2015')
                || ($usr == 'chenxuejun' && $pwd == 'hwcxj2015')
                || ($usr == 'zhangyang' && $pwd == 'hwzy2015')
                || ($usr == 'haojiaojiao' && $pwd == 'haojiaojiao2016')
                ||($usr == 'huli' && $pwd == 'huli2015')) {
                $_SESSION['login'] = true;
    ...
    private function _getToken() {
        // for ($i = 0; $i < 31; $i++) {
        //     echo "'" . date('d', strtotime("+$i days", strtotime(date('Y-m-d')))) . "' => '" . md5(createUserId(20)) . "',<br/>";
        // }
        $token = array(
